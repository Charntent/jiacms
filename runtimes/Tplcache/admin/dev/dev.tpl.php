...
<script type="text/javascript" src="<?php echo _STATIC; ?>/artdialog/jquery.artDialog.js?skin=black"></script>
<script type="text/javascript" src="<?php echo _STATIC; ?>/artdialog/plugins/iframeTools.source.js"></script>
<script type="text/javascript" src="<?php echo _STATIC; ?>/js/common.js"></script>
<script>
function dev_set(){
...
function setGroups(){
	 ...
	 $.post("<?php echo U($m.'/dev') ?>?action=setgroups",{},function(data){
		 ...
		 art.dialog({content:data,ok:function(){
			  ...
			  $.post("<?php echo U($m.'/dev') ?>?action=setgroups_do",{doids:str,type:groupid},function(data){
				   var list = art.dialog.list;
				   for (var i in list) {
		   			   list[i].close();
		           };
	...
	var ids = $("input[name=id]:checked"); 
	 ...
	 if(ids.length == 0) {
      ...
      art.dialog({content:'你真的要删除吗？删除了就不能恢复！如果你删除了上级，下级不选也会一起删除！',ok:function(){
			  ...
			  $.post("<?php echo U($m.'/dev') ?>?action=setdelall",{doids:str},function(data){
				   var list = art.dialog.list;
				   for (var i in list) {
		   			   list[i].close();
		           };
   ...
   <div class="bs-glyphicons">
    <ul class="bs-glyphicons-list">
         <li onClick="$('.bootstrap_icon').fadeOut();">
          <img src="<?php echo $skins_admin ?>/close.png">
	...
	<table border="0" cellspacing="0" cellpadding="0" class="form">
		...
		<tr>
			...
			<td>
                ...
                <?php echo fetchZiDianModel(0,0,$pid); ?>
				</select>
			</td>
		</tr>	
		<tr>
        ...
        <tr style="display:none">
			<td width="90">栏目标识</td>
			<td><input name="ident" type="text" class="form-control" style="width:200px;display:inline-block" value="<?php echo $ident_auto ?>">
		 ...
		 <tr>
			...
			<td>
				<select name="catgroup"  class="form-control" style="width:200px;">
				    ...
				    <?php Tag::var_protect("IN"); if(is_array($cats_groups)) foreach($cats_groups as $k=> $v) { ?>
...
<?php if($action=='edit') { ?>
	...
	<table border="0" cellspacing="0" cellpadding="0" class="form">
		<tr>
			...
			<?php } else { ?>
				<select name="parentid" onChange="return false; "  class="form-control" style="width:200px">
					<option value="0">顶级栏目</option>
					<?php echo fetchZiDianModel(0,0,$r["parentid"]); ?>
				</select>
			<?php } ?>
			</td>
		</tr>	
		<tr>
			<td width="90">栏目名称：</td>
			<td><input name="catname" type="text" style="width:400px;" value="<?php echo $r['catname'] ?>"  class="form-control"></td>
		</tr>
        <tr style="display:none">
			<td width="90">栏目标识（用于多版本的通信）</td>
			<td><input name="ident" type="text" value="<?php echo $r['ident'] ?>" disabled class="form-control" style="width:400px;"></td>
		</tr>
          <?php if($debug) { ?>
          <tr>
			<td width="90">栏目备注（其他语言看不懂时可以看这个）</td>
			<td><input name="transname" type="text" value="<?php echo $r['transname'] ?>" class="form-control" style="width:400px;"></td>
		</tr>
       <?php } ?>
        <tr>
			<td width="90">自定义栏目url（不能相同）</td>
			<td><input name="dir" type="text" value="<?php echo $r['dir'] ?>"  class="form-control" style="width:200px; display:inline-block">
            <div style=" display:inline-block;padding:10px 0; font-weight:bold; color:#f30">
               (只能用英文字母、数字作为url，用于优化seo，而且同语言不能相同！）
             </div>
             <label><input name="addpare" type="checkbox" value="1" <?php if($r['addpare']==1) { ?> checked<?php } ?>> （勾选表示url会续上上级栏目，比如上级是news,填news1表示该栏目的url为news/news1，如果不勾选，那么该栏目的url为news1）</label>
            </td>
		</tr>
         <tr>
			<td width="90">左边的图标</td>
			<td><input name="icon" type="text" value="<?php echo $r['icon'] ?>" class="form-control" style="width:200px; float:left" id="icon">
              <span class="glyphicon <?php echo $r['icon'] ?>" style="display:block; line-height:34px; float:left; padding-left:10px; font-size:20px;" id="glyphicon_icon"></span>
            <a href="javascript:void(0)" onClick="$('.bootstrap_icon').show();" class="fl" style="line-height:34px; padding-left:10px;">更换图标</a>
            </td>
		</tr>
		...
		<tr>
			...
			<td>
				<select name="cattype" onChange="changetype(this.value);"  class="form-control" style="width:200px;">
					<option value="0">请选择</option>
					<option value="article" <?php if($r['cattype']=='article') { ?>selected<?php } ?>>文章栏目</option>
					<option value="page" <?php if($r['cattype']=='page') { ?>selected<?php } ?>>独立页面</option>
					<option value="diypage" <?php if($r['cattype']=='diypage') { ?>selected<?php } ?>>自定义处理文件</option>
				</select>
			</td>
		</tr>
          ...
          <tr style="display:none" class="article_tr page_tr">
			<td width="90">模板文件：</td>
			<td><input name="template" type="text" style="width:400px;"   class="form-control" value="<?php echo $r['template'] ?>">  注：文章栏目设置为 栏目模板名|文章模板名，不含 .tpl.php</td>
		</tr>
        <tr style="display:none" class="diypage_tr">
			<td width="90">处理程序：</td>
			<td> 
				后台 <input name="phpscript[0]" type="text" style="width:180px;" value="<?php echo @$phpscript['0'] ?>"  class="form-control">
				前台 <input name="phpscript[1]" type="text" style="width:180px;" value="<?php echo @$phpscript['1'] ?>"  class="form-control">
			</td>
		</tr>
...
全功能编辑器|editor_id|editor<br>
添加到主字段|field1|input|主字段field1~field5可作为查询条件</div>
			<textarea name="fields" style="width:695px; height:160px;"  class="form-control"><?php echo $r['fields'] ?></textarea> </td>
		</tr>
        <?php } ?>
         <tr style="display:none" class="article_tr">
			<td width="90">排序</td>
            ...
            <label><input name="e_sort" type="checkbox" value="1" <?php if($r['e_sort']=='1') { ?>checked<?php } ?>> 是否允许自定义排序</label></td>
		</tr>
		 <?php if($debug) { ?>
		<tr style="display:none" class="article_tr">
			<td width="90">小分类：</td>
			<td><textarea name="subtype" style="width:450px; height:120px;"  class="form-control"><?php echo $r['subtype'] ?></textarea> 分类标识|分类名称，多个换行<br>
			<input name="e_subtype" type="hidden" value="0">
			<label><input name="e_subtype" type="checkbox" value="1" <?php if($r['e_subtype']=='1') { ?>checked<?php } ?>>允许编辑分类</label></td>
		</tr>
        ...
        <tr style="display:none" class="article_tr">
			<td width="90">是否是封面：</td>
			<td>
			<label><input name="isend" type="checkbox" value="1" <?php if($r['isend']=='1') { ?>checked<?php } ?>>封面不能添加文章</label></td>
		</tr>
		  ...
		  <tr>
			...
			<td>
				<select name="catgroup"  class="form-control" id="catgroup"  onChange="changegroup(this.value);" style="width:200px;">
					<option value="">不分组</option>
				    <?php Tag::var_protect("IN"); if(is_array($cats_groups)) foreach($cats_groups as $k=> $v) { ?>
                    <option value="<?php echo $k ?>" <?php if($r['catgroup']==$k) { ?> selected<?php } ?>><?php echo $v ?></option>
                    <?php };  Tag::var_protect("OUT"); ?>
				</select>
         ...
         <tr <?php if(!$r['catgroup']) { ?>style="display:none"<?php } ?> class="istongbu_tr" id="istongbu_tr">
...
$(function(){
   $(".ui4_radio li").click(function(){
		 ...
		 if($(this).data('type')=='yes'){
			 ...
			 $.post("<?php echo U($m.'/dev') ?>",{action:'getroups',catgroupid:catgroup},function(data){
				var list = art.dialog.list;
				for (var i in list) {
					list[i].close();
				};
...
<?php if($action=='group') { ?>
	...
	<form method="post">
		<table border="0" cellspacing="0" cellpadding="0" class="form" >
			...
			<?php Tag::var_protect("IN"); if(is_array($data)) foreach($data as $k=> $r) { ?>
			<tr>
				<td width="180"><input name="data[]" type="text" value="<?php echo $r['name'] ?>" style="width:120px;"  class="form-control"></td>
				<td width="300"><select name="data[]"  class="form-control" ><option value="1" <?php if($r['show']==1) { ?>selected<?php } ?>>默认展开</option><option value="0" <?php if($r['show']==0) { ?>selected<?php } ?>>默认隐藏</option></select>
              </td>
               <td><input name="data[]" type="text" value="<?php echo $r['icon'] ?>" style="display:inline; width:150px"  class="form-control"> <a href="javascript:void(0)" onClick="d(this);" class="btn btn-danger">删除</a></td>
			</tr>
			<?php };  Tag::var_protect("OUT"); ?>
			<tr class="table_add">
