...
"month":5,
"year":6 };
var topY=0,leftX=0; //自定义定位偏移量 2012-02-10 由 Intdar添加
//选择日期 → 由 Intdar 2012-02-10 添加，通过 ID 来选日期
...
2012-02-15 由 Intdar 修改为根据用户指定的 style 来确定；
*/
...
"sel_bg" : "#FFCCCC", //已被选择的日期单元格背影色 2012-02-03 Intdar添加
...
*/
//this.changeSelect();
//this.bindData(); //2012-02-15 由Indar注释
}
/**//**//**//**//**//**//**//**
* 日历类属性（语言包，可自由扩展）
*/
...
calendar = this;
var mvAry = [];
//mvAry[mvAry.length] = ' <form name="calendarForm" style="margin: 0px;">'; //因 <form> 不能嵌套， 2012-02-11 由Intdar改用 Div
...
mvAry[mvAry.length] = ' <th align="center" width="98%" nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:12px;width:60px"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;';
if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏"月"
mvAry[mvAry.length] = 'width:60px"></select></th>';
...
{ calendar.ReturnDate(""); /*calendar.dateControl.value = "";calendar.hide();*///2012-02-14 由Intdar注释
}
...
var cy = this.calendarYear;//2012-02-10 由Intdar修改
cy.length = 0;
...
var cm = this.calendarMonth;//2012-02-10 由Intdar修改
cm.length = 0;
for (var i = 0; i < 12; i++){
...
var cM = this.calendarMinute;
var cs = this.calendarSecond;
//2011-10-20 由Intdar修改，减少运算次数 
var $cyIn = this.endYear-this.date.getFullYear(); 
$cyIn = Math.abs($cyIn);
cy[$cyIn].selected = true;
cm[this.date.getMonth()].selected =true;
//2011-10-20 添加，初始化时间的值
...
this.year = e.calendarYear.options[e.calendarYear.selectedIndex].value;//2012-02-10 由Intdar修改
...
//2012-02-30 由民工.砖家修改 在Firefox 下年份错误
...
tds[i].style.cursor ="pointer"; //2007-08-06 由Intdar添加，鼠标变成手指状
...
var now = new Date();
this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle) ;//2008-01-29 Intdar添加 → 若为空则根据dateFormatStyle初始化日期
...
this.panel.style.left = (xy.x + leftX)+ "px"; //由Intdar 2007-02-11 修改 → 加入自定义偏移量
this.panel.style.top = (xy.y + topY + dateObj.offsetHeight) + "px";
//由Intdar 2006-06-25 修改 → 把 visibility 变为 display，并添加失去焦点的事件 //this.setDisplayStyle("select", "hidden");
...
this.dateControl.blurEvent = this.dateControl.onblur;}//2012-02-14 保存主文本框的 onblur ，使其原本的事件不被覆盖
...
//焦点转移时隐藏日历 → 由Intdar 2006-06-25 添加
Calendar.prototype.onblur = function() {
if(!isFocus){this.hide();}
}
//以下由Intdar 2007-07-26 修改 → 确保日历容器节点在 body 最后，否则 FireFox 中不能出现在最上方
